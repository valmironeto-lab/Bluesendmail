...
class BSM_Admin {
	...
	public function setup_admin_menu() {
		...
		add_menu_page( __( 'BlueSendMail', 'bluesendmail' ), __( 'BlueSendMail', 'bluesendmail' ), 'manage_options', 'bluesendmail', array( $this, 'render_dashboard_page' ), 'dashicons-email-alt2', 25 );
		add_submenu_page( 'bluesendmail', __( 'Dashboard', 'bluesendmail' ), __( 'Dashboard', 'bluesendmail' ), 'manage_options', 'bluesendmail', array( $this, 'render_dashboard_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Campanhas', 'bluesendmail' ), __( 'Campanhas', 'bluesendmail' ), 'manage_options', 'bluesendmail-campaigns', array( $this, 'render_campaigns_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Criar Nova Campanha', 'bluesendmail' ), __( 'Criar Nova', 'bluesendmail' ), 'manage_options', 'bluesendmail-new-campaign', array( $this, 'render_add_edit_campaign_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Contatos', 'bluesendmail' ), __( 'Contatos', 'bluesendmail' ), 'manage_options', 'bluesendmail-contacts', array( $this, 'render_contacts_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Listas', 'bluesendmail' ), __( 'Listas', 'bluesendmail' ), 'manage_options', 'bluesendmail-lists', array( $this, 'render_lists_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Importar', 'bluesendmail' ), __( 'Importar', 'bluesendmail' ), 'manage_options', 'bluesendmail-import', array( $this, 'render_import_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Relatórios', 'bluesendmail' ), __( 'Relatórios', 'bluesendmail' ), 'manage_options', 'bluesendmail-reports', array( $this, 'render_reports_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Logs do Sistema', 'bluesendmail' ), __( 'Logs do Sistema', 'bluesendmail' ), 'manage_options', 'bluesendmail-logs', array( $this, 'render_logs_page' ) );
		add_submenu_page( 'bluesendmail', __( 'Configurações', 'bluesendmail' ), __( 'Configurações', 'bluesendmail' ), 'manage_options', 'bluesendmail-settings', array( $this, 'render_settings_page' ) );
	}
	...
	public function handle_actions() {
		...
		if ( isset( $_GET['page'] ) && strpos( $_GET['page'], 'bluesendmail' ) !== false ) {
			$this->plugin->db->check_database_setup();
	...
	public function render_dashboard_page() {
		...
		$top_openers = $this->get_top_openers();
		$top_clickers = $this->get_top_clickers();
		?>
		<div class="wrap bsm-dashboard-wrap">
			...
			<div class="bsm-dashboard-grid bsm-main-grid">
				<div class="bsm-card">
					<h3 class="bsm-card-title"><?php _e( 'Performance Geral (Todas as Campanhas)', 'bluesendmail' ); ?></h3>
					<div class="bsm-chart-container">
						<canvas id="bsm-performance-chart"></canvas>
					</div>
				</div>
				<div class="bsm-card">
					<h3 class="bsm-card-title"><?php _e( 'Contatos Mais Engajados', 'bluesendmail' ); ?></h3>
					<div class="bsm-ranking-tabs">
						<span class="bsm-tab active" data-tab="openers"><?php _e( 'Maiores Leitores', 'bluesendmail' ); ?></span>
						<span class="bsm-tab" data-tab="clickers"><?php _e( "Maiores 'Clicadores'", 'bluesendmail' ); ?></span>
					</div>

					<div id="bsm-tab-openers" class="bsm-tab-content active">
						<?php if ( ! empty( $top_openers ) ) : ?>
							<ol>
								<?php foreach ( $top_openers as $contact ) : ?>
									<li><?php echo esc_html( $contact->email ); ?> <code>(<?php echo sprintf( _n( '%d abertura', '%d aberturas', $contact->open_count, 'bluesendmail' ), $contact->open_count ); ?>)</code></li>
								<?php endforeach; ?>
							</ol>
						<?php else : ?>
							<p><?php _e( 'Ainda não há dados de aberturas para exibir.', 'bluesendmail' ); ?></p>
						<?php endif; ?>
					</div>

					<div id="bsm-tab-clickers" class="bsm-tab-content">
						<?php if ( ! empty( $top_clickers ) ) : ?>
							<ol>
								<?php foreach ( $top_clickers as $contact ) : ?>
									<li><?php echo esc_html( $contact->email ); ?> <code>(<?php echo sprintf( _n( '%d clique', '%d cliques', $contact->click_count, 'bluesendmail' ), $contact->click_count ); ?>)</code></li>
								<?php endforeach; ?>
							</ol>
						<?php else : ?>
							<p><?php _e( 'Ainda não há dados de cliques para exibir.', 'bluesendmail' ); ?></p>
						<?php endif; ?>
					</div>
				</div>
			</div>
			...
			<div class="bsm-dashboard-grid">
				<div class="bsm-card bsm-card-full">
					<h3 class="bsm-card-title"><?php _e( 'Crescimento de Contatos (Últimos 30 dias)', 'bluesendmail' ); ?></h3>
					<div class="bsm-chart-container-large">
			...
			<div class="bsm-dashboard-grid">
				...
				<div class="bsm-card">
					...
					if ( $last_campaign ) :
						...
						<p><strong><?php _e( 'Estatísticas:', 'bluesendmail' ); ?></strong> <?php printf( __( '%d enviados, %d aberturas (%s%%), %d cliques (%s%% CTOR)', 'bluesendmail' ), $sent, $unique_opens, number_format_i18n( $open_rate, 2 ), $unique_clicks, number_format_i18n( $click_rate, 2 ) ); ?></p>
		...
		<div class="wrap">
			...
			<form method="post">
				...
				<div class="submit" style="padding-top: 10px;">
					<?php submit_button( $campaign ? __( 'Salvar Alterações', 'bluesendmail' ) : __( 'Salvar Rascunho', 'bluesendmail' ), 'secondary', 'bsm_save_draft', false ); ?>
	...
	public function render_settings_page() {
		...
		<div class="wrap">
			...
			<form method="post" action="options.php"><?php settings_fields( 'bluesendmail_settings_group' ); do_settings_sections( 'bluesendmail-settings' ); submit_button(); ?></form>
			<hr>
			<div class="bsm-card">
				...
				<ol>
					...
					<li><?php _e( 'No campo de comando, insira a seguinte linha:', 'bluesendmail' ); ?><pre style="background:#eee; padding:10px; border-radius:4px;"><code>wget -q -O - <?php echo esc_url( site_url( 'wp-cron.php?doing_wp_cron' ) ); ?> >/dev/null 2>&1</code></pre></li>
					<li><?php printf( __( 'Após configurar o cron job no servidor, adicione a seguinte linha ao seu arquivo %s para desativar o WP-Cron padrão:', 'bluesendmail' ), '<strong>wp-config.php</strong>' ); ?><pre style="background:#eee; padding:10px; border-radius:4px;"><code>define('DISABLE_WP_CRON', true);</code></pre></li>
				</ol>
			</div>
			<hr>
			<div id="bsm-system-status">
				...
				<table class="form-table">
					<tbody>
						<tr><th scope="row"><?php _e( 'Itens na Fila de Envio', 'bluesendmail' ); ?></th><td><?php global $wpdb; echo '<strong>' . esc_html( $wpdb->get_var( "SELECT COUNT(queue_id) FROM {$wpdb->prefix}bluesendmail_queue WHERE status = 'pending'" ) ) . '</strong>'; ?><p class="description"><?php _e( 'Número de e-mails aguardando na fila para serem enviados.', 'bluesendmail' ); ?></p></td></tr>
						<tr><th scope="row"><?php _e( 'Próxima Execução da Fila', 'bluesendmail' ); ?></th><td><?php $timestamp = wp_next_scheduled( 'bsm_process_sending_queue' ); echo $timestamp ? '<strong>' . get_date_from_gmt( date( 'Y-m-d H:i:s', $timestamp ), 'd/m/Y H:i:s' ) . '</strong>' : '<strong style="color:red;">' . esc_html__( 'Não agendado!', 'bluesendmail' ) . '</strong>'; ?><p class="description"><?php _e( 'A próxima vez que o WordPress irá tentar processar a fila de envio.', 'bluesendmail' ); ?></p></td></tr>
						<tr>
							<th scope="row"><?php _e( 'Última Execução Realizada', 'bluesendmail' ); ?></th>
							<td>
								<?php
								$last_run = get_option( 'bsm_last_cron_run' );
								if ( $last_run ) echo '<strong>' . get_date_from_gmt( date( 'Y-m-d H:i:s', $last_run ), 'd/m/Y H:i:s' ) . '</strong> (' . sprintf( esc_html__( '%s atrás' ), human_time_diff( $last_run ) ) . ')';
								else echo '<strong>' . esc_html__( 'Nunca executado', 'bluesendmail' ) . '</strong>';
								if ( $last_run && ( time() - $last_run > 30 * MINUTE_IN_SECONDS ) ) echo '<p style="color: #a00;"><strong>' . esc_html__( 'Atenção:', 'bluesendmail' ) . '</strong> ' . esc_html__( 'A última execução do agendador foi há muito tempo. Isso pode indicar que o WP-Cron não está a funcionar corretamente no seu servidor. Para garantir a máxima fiabilidade no envio, recomendamos configurar um cron job no seu painel de alojamento.', 'bluesendmail' ) . '</p>';
								?>
			...
			<form method="post">
				<table class="form-table"><tr valign="top"><th scope="row"><label for="bsm_test_email_recipient"><?php _e( 'Enviar para', 'bluesendmail' ); ?></label></th><td><input type="email" id="bsm_test_email_recipient" name="bsm_test_email_recipient" value="<?php echo esc_attr( wp_get_current_user()->user_email ); ?>" class="regular-text" required><p class="description"><?php _e( 'O e-mail de teste será enviado para este endereço.', 'bluesendmail' ); ?></p></td></tr></table>
	...
	private function render_report_content( $campaign_id ) {
		?>
		<div id="bsm-reports-summary" style="margin-top: 20px;"><div class="bsm-dashboard-grid"><div class="bsm-card"><div class="bsm-chart-container" style="max-width: 450px; margin: auto;"><canvas id="bsm-report-chart"></canvas></div></div></div></div>
	...
	private function render_reports_selection_page() {
		...
		<div class="bsm-card" style="margin-top:20px;">
			<h3 class="bsm-card-title"><?php esc_html_e( 'Selecione uma Campanha', 'bluesendmail' ); ?></h3><p><?php esc_html_e( 'Escolha uma campanha abaixo para visualizar o seu relatório detalhado.', 'bluesendmail' ); ?></p>
	...
	public function render_logs_page() {
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php echo esc_html__( 'Logs do Sistema', 'bluesendmail' ); ?></h1><hr class="wp-header-end">
	...
	public function render_import_page() {
		...
		if ( 2 === $step && isset( $_POST['bsm_import_step1'] ) ) $this->render_import_step2();
		else $this->render_import_step1();
	}
	...
	private function render_import_step1() {
		...
		?>
		<div class="wrap">
			<h1><?php _e( 'Importar Contatos - Passo 1 de 2', 'bluesendmail' ); ?></h1><p><?php _e( 'Selecione um arquivo CSV para enviar. O arquivo deve conter uma linha de cabeçalho com os nomes das colunas (ex: "E-mail", "Nome", "Apelido").', 'bluesendmail' ); ?></p>
	...
	private function render_import_step2() {
		...
		?>
		<div class="wrap">
			<h1><?php _e( 'Importar Contatos - Passo 2 de 2', 'bluesendmail' ); ?></h1><p><?php _e( 'Associe as colunas do seu arquivo CSV aos campos de contato do BlueSendMail.', 'bluesendmail' ); ?></p>
	...
	public function render_lists_page() {
		$action = isset( $_GET['action'] ) ? sanitize_key( $_GET['action'] ) : '';
		if ( 'new' === $action || 'edit' === $action ) $this->render_add_edit_list_page();
		else $this->render_lists_list_page();
	}
	...
	public function render_lists_list_page() {
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php echo esc_html__( 'Listas de Contatos', 'bluesendmail' ); ?></h1><a href="<?php echo esc_url( admin_url( 'admin.php?page=bluesendmail-lists&action=new' ) ); ?>" class="page-title-action"><?php echo esc_html__( 'Adicionar Nova', 'bluesendmail' ); ?></a><hr class="wp-header-end">
	...
	public function render_add_edit_list_page() {
		...
		?>
		<div class="wrap">
			<h1><?php echo $list ? esc_html__( 'Editar Lista', 'bluesendmail' ) : esc_html__( 'Adicionar Nova Lista', 'bluesendmail' ); ?></h1>
			<form method="post">
	...
	public function render_contacts_page() {
		$action = isset( $_GET['action'] ) ? sanitize_key( $_GET['action'] ) : '';
		if ( 'new' === $action || 'edit' === $action ) $this->render_add_edit_contact_page();
		else $this->render_contacts_list_page();
	}
	...
	public function render_contacts_list_page() {
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php echo esc_html__( 'Contatos', 'bluesendmail' ); ?></h1><a href="<?php echo esc_url( admin_url( 'admin.php?page=bluesendmail-contacts&action=new' ) ); ?>" class="page-title-action"><?php echo esc_html__( 'Adicionar Novo', 'bluesendmail' ); ?></a><hr class="wp-header-end">
	...
	public function render_add_edit_contact_page() {
		...
		?>
		<div class="wrap">
			<h1><?php echo $contact ? esc_html__( 'Editar Contato', 'bluesendmail' ) : esc_html__( 'Adicionar Novo Contato', 'bluesendmail' ); ?></h1>
			<form method="post">
				<table class="form-table" role="presentation">
					<tbody>
	...
	public function handle_import_contacts() {
		...
		if ( ! current_user_can( 'manage_options' ) ) wp_die( esc_html__( 'Você não tem permissão para realizar esta ação.', 'bluesendmail' ) );
	...
	public function handle_send_test_email() {
		...
		$mailer_type = $this->plugin->options['mailer_type'] ?? 'wp_mail';
		$result = 'sendgrid' === $mailer_type ? $this->plugin->send_via_sendgrid( $recipient, $subject, $body ) : $this->plugin->send_via_wp_mail( $recipient, $subject, $body );
		if ( $result ) {
